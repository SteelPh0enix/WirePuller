...
void JsonAxisController::parseJsonInput(ArduinoJson::JsonDocument const& input,
  ...
  char const* requestType =
      input[Constant::Json::Key::RequestType].as<char const*>();

  if (requestType == NULL) {
    output[Constant::Json::Key::RequestError] =
        Constant::Json::Value::RequestError::NoRequestType;
  } else if (strcmp(requestType,
                    Constant::Json::Value::RequestType::Callibrate) == 0) {
  } else if (strcmp(requestType,
                    Constant::Json::Value::RequestType::SetPower) == 0) {
  } else {
    output[Constant::Json::Key::RequestError] =
